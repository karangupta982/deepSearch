import { Fragment, useMemo, useState, Dispatch, SetStateAction, useCallback } from "react";
...
export function APIKeyModal({ open, onClose, apiKey, setApiKey }: APIKeyModalProps) {
    ...
    const validate = useCallback(
        async (key: string) => {
            ...
            if (key.length === 0) {
                ...
                setErrorMessage("API key should start with 'AI'");
            } else {
                // actual validation by pinging Gemini's API
                setLoading(true);
                try {
                        ...
                        >
                            <Dialog.Panel className="relative max-w-[450px] transform overflow-hidden rounded-lg bg-gray-700 px-4 pb-4 text-left shadow-xl transition-all sm:my-8 sm:p-6">
                                ...
                                <div className="mt-2 text-xs text-gray-400">
                                    Your API key is sent directly to Google's Gemini API from your browser; it never touches our servers!
                                </div>
                            </Dialog.Panel>
                        </Transition.Child>
                    </div>
